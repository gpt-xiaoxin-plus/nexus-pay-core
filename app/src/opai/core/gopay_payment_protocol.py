...
class GoPayPayment:
    ...
    def _midtrans_get(self, path: str, timeout: int = 15) -> dict:
        ...
        r = self._session.get(url, headers=self._request_headers(), timeout_seconds=timeout)
    ...
    def _midtrans_post(self, path: str, body: dict, timeout: int = 15) -> dict:
        url = f"{MIDTRANS_BASE}{path}"
        r = self._session.post(url, headers=self._request_headers(), data=json.dumps(body), timeout_seconds=timeout)
    ...
    def _midtrans_delete(self, path: str, timeout: int = 15) -> dict:
        url = f"{MIDTRANS_BASE}{path}"
        r = self._session.delete(url, headers=self._request_headers(), timeout_seconds=timeout)
    ...
    def pay(
        ...
        for attempt in range(1, LINK_RETRY_LIMIT + 2):
            link_r = self._midtrans_post(f"/snap/v3/accounts/{snap}/linking", link_body)
